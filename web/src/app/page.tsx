...
const LAYER_BORDER_COLORS: Record<string, string> = {
  ...
  intelligence: "border-emerald-500/30 hover:border-emerald-500/60",
  extension: "border-amber-500/30 hover:border-amber-500/60",
  production: "border-red-500/30 hover:border-red-500/60",
};
...
function getVersionData(id: string) {
  return (versionsData as any).versions?.find((v: any) => v.id === id);
}
    ...
    <div className="flex flex-col gap-20 pb-16">
        ...
        <div className="mx-auto max-w-2xl overflow-hidden rounded-xl border border-zinc-800 bg-zinc-950">
          ...
          <pre className="overflow-x-auto p-4 text-sm leading-relaxed">
            <code>
              ...
              <span className="text-blue-400">execute_tools</span>
              <span className="text-zinc-500">(</span>
              <span className="text-zinc-300">response.tool_calls</span>
              <span className="text-zinc-500">)</span>
              {"\n"}
      ...
      <section>
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {LEARNING_PATH.map((versionId) => {
            ...
            if (!meta) return null;
            return (
              <Link key={versionId} href={`/learn/${versionId}`} className="group block">
                <Card className={cn("h-full border transition-all duration-200", LAYER_BORDER_COLORS[meta.layer])}>
                  ...
                  <h3 className="mt-3 text-sm font-semibold group-hover:underline">{meta.title}</h3>
      ...
      <section>
        ...
        <div className="flex flex-col gap-3">
          {LAYERS.map((layer) => (
            <div key={layer.id} className="flex items-center gap-4 rounded-xl border border-[var(--color-border)] bg-[var(--color-bg)] p-4">
              <div className={cn("h-full w-1.5 self-stretch rounded-full", LAYER_BAR_COLORS[layer.id])} />
              <div className="flex-1">
                ...
                <div className="mt-2 flex flex-wrap gap-1.5">
                  {layer.versions.map((vid) => {
                    ...
                    return (
                      <Link key={vid} href={`/learn/${vid}`}>
                        <LayerBadge layer={layer.id} className="cursor-pointer transition-opacity hover:opacity-80">
                          {vid}: {meta?.title}
                        </LayerBadge>
                      </Link>
                    );
