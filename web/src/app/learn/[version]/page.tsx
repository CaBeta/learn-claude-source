...
export default async function VersionPage({
  ...
  const versionData = (versionsData as any).versions?.find((v: any) => v.id === version);
  const meta = VERSION_META[version];
  const diff = (versionsData as any).diffs?.find((d: any) => d.to === version) ?? null;
    ...
    <div className="mx-auto max-w-3xl space-y-10 py-4">
      ...
      <header className="space-y-3">
        ...
        <div className="flex flex-wrap items-center gap-4 text-sm text-zinc-500 dark:text-zinc-400">
          ...
          <span>{versionData.tools?.length || 0} 个工具</span>
          {meta.coreAddition && (
        ...
        {prevVersion ? (
          ...
          >
            ...
            <div>
              ...
              <div className="font-medium">{prevVersion} - {VERSION_META[prevVersion]?.title}</div>
            </div>
          </Link>
        ) : <div />}
        {nextVersion ? (
          <Link
            href={`/learn/${nextVersion}`}
          ...
          >
            <div>
              ...
              <div className="font-medium">{VERSION_META[nextVersion]?.title} - {nextVersion}</div>
            </div>
